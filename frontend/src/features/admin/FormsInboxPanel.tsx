...
export function FormsInboxPanel() {
  ...
  return <><PageHeader kicker="Admin" title="Forms inbox" sub="Contact requests and event suggestions share one synchronized data view." /><div className="admin-filterbar"><input aria-label="Search forms" type="search" value={query} placeholder="Search name, email, or reference" onChange={(event) => setQuery(event.target.value)} /><select aria-label="Form type" value={type} onChange={(event) => setType(event.target.value)}><option value="">All forms</option><option value="contact">Contact</option><option value="suggestion">Suggestions</option></select><select aria-label="Form status" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All statuses</option>{STATUSES.map((value) => <option key={value} value={value}>{value.replaceAll("_", " ")}</option>)}</select></div><DataViews items={state.data.items} keyFor={(item) => `${item.type}-${item.id}`} columns={["Reference", "Contact", "Subject", "Status", "Actions"]} renderCells={(item) => [item.publicId, <span>{item.name}<br/><a href={`mailto:${item.email}`}>{item.email}</a></span>, <span title={item.message}>{item.subject}</span>, <StatusBadge status={item.status} />, actions(item)]} renderCard={card} empty="No matching form submissions." /></>;
}
