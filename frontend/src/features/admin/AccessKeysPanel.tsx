...
export function QrScanner() {
  ...
  return <section className="card qr-scanner"><h2>Scan an access key</h2>{detectorClass ? <><div className="form-actions"><label className="btn btn-outline btn-sm">Upload QR image<input className="sr-only" type="file" accept="image/*" capture="environment" onChange={upload} /></label><button className="btn btn-outline btn-sm" type="button" onClick={camera} disabled={scanning}>Use camera</button>{scanning ? <button className="btn btn-ghost btn-sm" type="button" onClick={stopCamera}>Cancel camera</button> : null}</div><video ref={video} muted playsInline /></> : <p>Your browser does not support QR detection. Enter the key in the admin sidebar.</p>}{message ? <p className="notice notice-info" role="status">{message}</p> : null}</section>;
}
