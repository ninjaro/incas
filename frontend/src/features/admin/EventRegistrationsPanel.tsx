...
export function EventRegistrationsPanel() {
  ...
  const actions = (item: RegistrationRecord) => {
    ...
    return <select aria-label={`Status for ${item.name}`} value={item.status} disabled={choices.length === 1} onChange={(event) => update(item, event.target.value as EventRegistrationStatus)}>{choices.map((value) => <option key={value} value={value}>{value.replaceAll("_", " ")}</option>)}</select>;
  };
