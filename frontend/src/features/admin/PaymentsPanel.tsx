...
export function PaymentsPanel() {
  ...
  const card = (item: AdminPayment) => <><small>{item.publicId}</small><h3>{item.eventTitle}</h3><p>{item.registrationName} / {item.registrationPublicId}</p><StatusBadge status={item.status} /><p>{new Intl.NumberFormat(undefined, { style: "currency", currency: item.currency }).format(item.amountCents / 100)}</p>{actions(item)}</>;
  return <><PageHeader kicker="Admin" title="Payments" sub="Registration-linked transactions and refund states." /><div className="admin-filterbar"><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All statuses</option>{["pending", "paid", "failed", "cancelled", "refund_pending", "refunded"].map((value) => <option key={value} value={value}>{value.replaceAll("_", " ")}</option>)}</select></div><DataViews items={state.data.items} keyFor={(item) => item.id} columns={["Payment", "Event", "Registration", "Amount", "Status", "Actions"]} renderCells={(item) => [item.publicId, item.eventTitle, `${item.registrationName ?? "-"} (${item.registrationPublicId ?? "-"})`, new Intl.NumberFormat(undefined, { style: "currency", currency: item.currency }).format(item.amountCents / 100), <StatusBadge status={item.status} />, actions(item)]} renderCard={card} empty="No matching transactions." /></>;
}
